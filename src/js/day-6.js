...
let Coordinates = (xAxis, yAxis, state) => {
    return {
        "x" : xAxis,
        "y" : yAxis,
        "state": state 
...
let buildArray = (x, y, state) => {
	for (var i = 0; i <= x; i++) {
    	twinkles[i] = [];
    	for (var j = 0; j <= y; j++) {
        	twinkles[i][j] = Coordinates(i, j, state);
    	}
	}
};
...
let parseInput = (input) => {
    ...
    for (var i = 0; i < splitInput.length; i++) {
    	...
    	var currentbeginingX = undefined;
    	var currentbeginingY = undefined;
    	var currentEndingX = undefined;
    	var currentEndingY = undefined;
    	var currentState = undefined;

    	currentState = currentInput[0];
    	for (var a = 0; a < numbersInput.length; a++) {
    		var currentNum = Number(numbersInput[a]);
    		if(a === 0){
    			currentbeginingX = currentNum;
    		} else if(a === 1){
    			currentbeginingY = currentNum;
    		} else if(a === 2){
    	...
    	var instruction = new SingleInput(currentbeginingX, currentbeginingY, currentEndingX, currentEndingY, currentState);
    	resultInput.push(instruction);
    }
};

let changeState = (instruction) => {
	if (instruction.state === "toggle") {
		for (var i = instruction.beginingX; i <= instruction.endingX; i++) {
			for (var j = instruction.beginingY; j <= instruction.endingY; j++) {
				var currentState = twinkles[i][j].state;
				if (currentState === "on")	{
					twinkles[i][j].state = "off";
				} else {
					twinkles[i][j].state = "on";
				}
			}
		}
	} else {
		for (var i = instruction.beginingX; i <= instruction.endingX; i++) {
			for (var j = instruction.beginingY; j <= instruction.endingY; j++) {
				twinkles[i][j].state = instruction.state;
			}
		}
	}
};

let countTwinkles = (xLength, yLength) => {
	var lightCount = 0;
	for (var j = 0; j <= xLength; j++) {
		for (var k = 0; k <= yLength; k++) {
			if (twinkles[j][k].state === "on") {
				lightCount += 1;
			}
		}
	}
...
let mainFunc = (xLength, yLength, initialState, input) => {
	parseInput(input);
	buildArray(xLength, yLength, initialState);
	for (var i = 0; i < resultInput.length; i++) {
		changeState(resultInput[i]);
	}

	var result = countTwinkles(xLength, yLength);

	console.log(result);
};


mainFunc(999, 999, "off", input);